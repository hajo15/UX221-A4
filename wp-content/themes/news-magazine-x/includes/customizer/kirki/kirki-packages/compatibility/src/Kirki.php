...
class Kirki extends Init {
	...
	public static function add_field( $config_id, $args = [] ) {
		...
		if ( doing_action( 'customize_register' ) ) {
			_doing_it_wrong( __METHOD__, esc_html__( 'Kirki fields should not be added on customize_register. Please add them directly, or on init.', 'news-magazine-x' ), '3.0.10' );
		}
		...
		// by Duke: Fix Pro Fields
		if ( str_contains($classname, 'Inputslider') ) {
			$classname = str_replace('Field', 'Pro\Field', $classname);
		}
		if ( str_contains($classname, 'Kirki_Margin') ) {
			$classname = '\Kirki\Pro\Field\Margin';
		}
