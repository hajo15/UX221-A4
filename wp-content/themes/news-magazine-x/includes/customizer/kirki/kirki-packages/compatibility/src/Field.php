...
class Field {
    ...
    /**
     * by Duke: Add Properties to fix 8.3+ errors.
     */
    protected $compiler;
	protected $tab;
	...
	public function __construct( $config_id = 'global', $args = array() ) {
		...
		if ( isset( $args['setting'] ) && ! empty( $args['setting'] ) && ( ! isset( $args['settings'] ) || empty( $args['settings'] ) ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( 'Typo found in field %s - setting instead of settings.', 'news-magazine-x' ), esc_html( $args['settings'] ) ), '3.0.10' );
		...
		if ( '' === $config_id ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( 'Config not defined for field %1$s - See %2$s for details on how to properly add fields.', 'news-magazine-x' ), esc_html( $args['settings'] ), 'https://aristath.github.io/kirki/docs/getting-started/fields.html' ), '3.0.10' );
			$this->kirki_config = 'global';
		}
	
		// by Duke: php 8.3 fix.
		$this->compiler      = isset( $args['compiler'] ) ? $args['compiler'] : [];
		$this->tab           = isset( $args['tab'] ) ? $args['tab'] : ''; // Default to an empty string
		$this->custom_class  = isset( $args['custom_class'] ) ? $args['custom_class'] : ''; // Default to an empty string
		$this->divider       = isset( $args['divider'] ) ? $args['divider'] : ''; // Default to an empty string
	...
	protected function set_option_type() {
		...
		if ( 'theme_mods' === $this->option_type ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html( 'Typo found in field %s - "theme_mods" vs "theme_mod"', 'news-magazine-x' ), esc_html( $this->settings ) ), '3.0.10' );
	...
	protected function set_partial_refresh() {
		...
		foreach ( $this->partial_refresh as $id => $args ) {
			if ( ! is_array( $args ) || ! isset( $args['selector'] ) || ! isset( $args['render_callback'] ) || ! is_callable( $args['render_callback'] ) ) {
				/* translators: %1$s represents the field ID where the error occurs. */
				_doing_it_wrong( __METHOD__, sprintf( esc_html__( '"partial_refresh" invalid entry in field %s', 'news-magazine-x' ), esc_html( $this->settings ) ), '3.0.10' );
	...
	protected function set_output() {
		...
		if ( ! is_array( $this->output ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( '"output" invalid format in field %s. The "output" argument should be defined as an array of arrays.', 'news-magazine-x' ), esc_html( $this->settings ) ), '3.0.10' );
			$this->output = array(
				array(
					'element' => $this->output,
				),
